/* Orthogonal Matching Persuits Implementation By Mehdi Safarpour,
   ...
   The current version is not suitible yet and can be improved to reducing
   redundancies in calculations 2017 In this version, the functions that require
   definition of arrays inside are inlinded in the main
   */
    ...
    void decompress(float *A, float *y, float *x_hat, int m, int n) {
        /* BEGINNIG OF OMP ALGORITHM */
        {
            ...
            for (int iii = 0; iii < 5; iii++)
                for (int ii = 0; ii < 4;
                        ii++) {  // main loop,  q is number of expected coefficients or
                    // iterations
                    //cout << iii << " " << ii << endl;
